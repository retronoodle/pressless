# Capability: web-installer

## Purpose

The web installer turns a freshly extracted release ZIP into a configured, migrated, and locked Stead install through a browser alone — no terminal, no hand-edited `.env`, no `git clone`. It is reachable only on a site that has never been installed and becomes permanently inert after the first successful install.
